# Word2JATS Frontend

Vue 3 前端提供 `.doc`/`.docx` 上传、批量转换、结构化审阅、人工校正、质量报告、原文映射、图表预览和结果导出。

## 当前支持

| 能力 | 状态 |
| --- | --- |
| `.doc` 与 `.docx` 单篇/批量上传 | 已支持 |
| DOCX 真实文档流解析与原文映射 | 已支持 |
| 图片、表格、列表、章节归属与可视化预览 | 已支持 |
| TIFF 原件保留与 PNG 在线预览 | 已支持 |
| 正文前辅助媒体保留与正文 `fig` 隔离 | 已支持 |
| OMML 转 MathML/LaTeX 与稳定降级 | 已支持 |
| 图、表、公式、数字制及作者年份制参考文献交叉引用恢复 | 已支持 |
| 参考文献细粒度解析与 `element-citation` | 已支持 |
| JATS Publishing 1.3 MathML3 DTD 校验 | 已支持 |
| 官方样例 XML 结构对比 | 已支持 |
| 结构证据、置信度、复核状态与 xref 目标摘要 | 已支持 |
| Schema 白名单自动修复与人工校正闭环 | 已支持 |
| 批量转换与 ZIP 结果包 | 已支持 |
| 转换运行统计与发布自检 | 已支持 |
| 上传限额提示与后端安全边界 | 已支持 |

## 技术栈

Vue 3、Vite、Element Plus、axios。

## 安装与启动

先启动后端 `http://127.0.0.1:8000`：

```bash
npm install
npm run dev
```

访问 `http://localhost:5173`。生产构建：

```bash
npm run build
```

也可在项目根目录执行 `docker compose up --build`，访问 `http://localhost:8080`。Docker 版 Nginx 与后端统一使用 220 MB 请求上限，后端另设单文件 50 MB、单批 20 个文件的默认限制，支持一次性上传 5 篇官方样例。超限请求会返回 HTTP 413 和明确提示。

## 页面能力

- Profile 选择、单篇或多篇 `.doc`/`.docx` 上传
- 一键加载官方样例，并展示与官方 XML 结果的结构对比
- 批量状态、质量分、错误数、警告数和导出状态
- 结构化 JSON 与 JATS XML 预览
- XML 复制、单篇下载和完整 ZIP 下载
- 人工校正标题、摘要、关键词、作者、单位、章节、图表、公式和参考文献
- XML 合法性、正式 JATS Schema、业务规则和交叉引用分区展示
- 质量总分、七项分项得分、结构证据置信度、xref 目标摘要、错误定位和修复建议
- OMML `success/partial/failed`、支持特性、问题、MathML 和 LaTeX 展示
- 文档流节点到 JATS 标签、路径、章节和目标 ID 的对照
- 图片缩略图、大图、表格前 10 行、题注、引用次数和 JATS 片段预览
- TIFF 图片自动显示 PNG 预览；原始 TIFF 保留在 JATS/ZIP 媒体交付中
- 正文开始前的图形摘要、作者照片等作为 `auxiliary_media` 保留在 JSON/ZIP 中，不占用正文 figure 编号
- 生成 XML 与官方 XML 的语义质量对比：总分、元数据、章节、图表、公式、参考文献、xref 和 XML 合规分项
- 转换运行摘要：耗时、源节点数、对象数量、校验错误数和自动修复轮次

## 官方样例演示

“一键加载官方样例”通过 `/api/demo-documents` 获取白名单清单并同时加载 5 篇官方第一组 Word 主稿：

```text
official-sample-1-group-1.docx
official-sample-2-group-1.docx
official-sample-3-group-1.docx
official-sample-4-group-1.docx
official-sample-5-group-1.docx
```

这些展示名映射到 `样例-最新版` 目录下的官方 Word 文件。转换完成后，“官方对比”Tab 会展示语义指标 V2 总分、七个分项、可恢复差异和出版方补录差异；“质量报告”会展示图表/公式证据状态和 xref 目标完整性。当前可复现结果为五篇平均 **96.6%**、最低 **95%**、JATS 1.3 DTD 通过率 **100%**。最新优化包含图表题注续行合并、图后风险人数小表识别、表注 `table-wrap-foot` 输出、caption 内交叉引用恢复、单位 `<aff><label>`、未编号一级章节 `<label>` 与编号章节 label/title 拆分输出、紧凑英文期刊参考文献尾部与 DOI URL 解析、逗号分隔英文期刊参考文献解析、结构化参考文献原始 `mixed-citation` 保真输出，以及 `fig1/tab9` 与 `fig001/tab009` 这类等价目标 ID 的语义归一。旧版 Word 预转换后的命名标题样式、正文内嵌公式图片和紧凑图例表均按文档流证据处理，减少章节扁平化、公式误判为插图和图例误判为正文表格。图片化表格采用文档流证据保守绑定，正文、caption 和原生表格单元格中的有效引用均会生成 JATS `<xref>`。

当前后端还支持区分正文中的图表引用叙述与真实题注，恢复旧版 Word 转换后的多行标题、偏移标题层级、姓名首字母作者行、Scheme 图题、图例小表和图片公式。图片公式在 JSON、XML、质量报告和文档流对照中显示为 `image_formula`，JATS 使用 `disp-formula/graphic` 保留原始媒体，并以 `partial` 状态提示人工补充 MathML/LaTeX；文末图形摘要保留为辅助媒体，不混入正文图片。作者单位及作者年份引用仍按结构证据恢复，未能同时匹配第一作者、年份和参考文献目标的内容保持原文，供人工复核。

## 使用流程

1. 选择 Profile 并上传 `.doc`/`.docx`，或一键加载官方样例；`.doc` 在后端完成受控预转换，Docker 版无需额外配置。
2. 从批量列表选择文章，查看 JSON、XML、校验和质量报告。
3. 使用“文档流对照”解释 DOCX 到 JATS 的映射。
4. 使用“图表预览”核对媒体、表格、题注、引用和 XML 片段。
5. 使用“官方对比”查看系统 XML 与官方 XML 的总分、分项得分和可定位差异。
6. 在“人工校正”补充或修正结构化字段，点击“重新生成 XML”。
7. 查看重新执行后的正式 Schema、业务规则、交叉引用和质量分。
8. 下载 XML 或 ZIP 交付包。

每篇结果的接口响应包含 `processing_stats`，用于复核单篇转换规模和耗时；它是审计摘要，不是跨机器性能承诺。

## 决赛展示流程

1. 一键加载官方样例，对比系统生成 XML 和官方 XML 结果。
2. 展示能力卡片与批量转换状态。
3. 展示文档流、图表、OMML、xref 和质量报告。
4. 人工补充出版元数据并重新生成。
5. 展示 Schema 状态变化和 ZIP 交付。
6. 发布前运行后端 `python scripts/release_check.py` 和前端 `npm run build`，确认演示环境与仓库内容一致。

## 当前真实限制

- 启发式规则面对复杂排版、多作者多单位映射时仍可能需要人工校正。
- `.doc` 依赖后端 LibreOffice 预转换；旧 Equation Editor/OLE 对象可能转成图片并标记复核，本机后端未安装 LibreOffice 时不能处理 `.doc`。
- 作者年份制引用在同作者同年歧义、缺失年份或非标准姓名顺序下可能保留原文并提示复核。
- 不保证覆盖全部 Office Math；复杂嵌套矩阵、复杂分段、多重重音和未知 OMML 子结构会标记 `partial/failed`。
- 图片公式暂不支持 OCR，系统不调用商业 API。
- 复杂合并单元格、跨页表格、嵌套列表和非标准参考文献仍需人工复核。
- `auxiliary_media` 仅隔离并保留正文前媒体，不自动决定图形摘要、作者照片等最终 JATS 出版位置。
- 正式 DTD 校验不会自动编造 ISSN、DOI、ORCID 等真实出版元数据。
- 当前批量转换为请求内顺序处理，生产环境仍可扩展持久化任务队列、鉴权和跨实例配额控制。
- 质量分是可解释的规则评分，不等同于出版社最终验收结论。
- 复杂嵌套字段仍主要通过 JSON textarea 编辑，尚未全部表单化。
- 暂未提供转换历史、异步任务进度和用户权限。
- 尚未完成完整国际化与可访问性审计。
