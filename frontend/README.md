...
当前后端还支持区分正文中的图表引用叙述与真实题注，恢复带普通数字/上标单位标记和学位后缀的作者，利用 Word `Affiliation` 样式补充单位，并恢复全名、`et al.`、叙述式及分号分隔的作者年份引用。前端会在 JSON、XML、校验结果、质量报告和文档流对照中展示这些结构；未能同时匹配第一作者、年份和参考文献目标的内容保持原文，供人工复核。
