...
- TIFF 图片自动显示 PNG 预览；原始 TIFF 保留在 JATS/ZIP 媒体交付中
- 生成 XML 与官方 XML 的语义质量对比：总分、元数据、章节、图表、公式、参考文献、xref 和 XML 合规分项
- 转换运行摘要：耗时、源节点数、对象数量、校验错误数和自动修复轮次
...
每篇结果的接口响应包含 `processing_stats`，用于复核单篇转换规模和耗时；它是审计摘要，不是跨机器性能承诺。
...
6. 发布前运行后端 `python scripts/release_check.py` 和前端 `npm run build`，确认演示环境与仓库内容一致。
