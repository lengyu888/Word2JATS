...
async def _convert_upload(file: UploadFile) -> dict:
...
async def convert_docx(file: UploadFile = File(...)) -> dict:
    try:
        return await _convert_upload(file)
...
async def batch_convert_docx(files: list[UploadFile] = File(...)) -> dict:
    results = []
    for file in files:
        filename = file.filename or "unnamed.docx"
        try:
            converted = await _convert_upload(file)
            results.append({
                "filename": filename,
                "status": "success",
                **converted,
