...
Word2JATS 是一个面向学术出版的智能结构化转换原型。用户上传 `.docx` 学术论文后，系统使用可解释的规则算法提取文章结构，生成中间 JSON 和 JATS XML，并执行正式 Schema、业务规则与交叉引用校验。
...
| JATS Publishing 1.4 正式 DTD 校验 | 已支持，官方 MathML3 DTD 已内置 |
| Schema 白名单自动修复 | 已支持 |
| 人工校正后重新生成与校验 | 已支持 |
| 批量转换与 ZIP 交付 | 已支持 |
...
- 复杂或非标准参考文献的细粒度字段可能解析不完整，仍需人工复核。
- 缺少 ISSN、完整贡献者信息等真实出版元数据时，正式 DTD 校验仍会失败；系统不会自动编造这些数据。
